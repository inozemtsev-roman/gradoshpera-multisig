...
const renderCurrentMultisigInfo = (): void => {
  ...
  for (const lastOrder of lastOrders) {
    if (lastOrder.type == "executed") {
      ...
      if (!wasPending) {
        lastOrdersHTML += '<div class="label">Ожидающие заявки:</div>';
        wasPending = true;
      }
    }
...
const daoMultisigsBlock = $("#daoMultisigsBlock");
const daoMultisigsList = $("#daoMultisigsList");

let daoRenderSeq = 0;

const rawOfRegistry = (friendly: string): string => {
  try {
...
const daoMultisigItemHTML = (
  ...
  if (status.ok) {
    if (status.isMultisig) {
      line = `${makeAddressLink(info)} — ${fromNano(BigInt(status.balance || "0"))} GRAM${roleBadgeHTML(status.role)}`;
    } else {
...
const renderDaoMultisigs = async (force = false): Promise<void> => {
  ...
  const items: { name: string; address: string; mine: boolean }[] = [];
  ...
  for (const entry of entries) {
    ...
    seen.add(raw);
    items.push({ name: entry.name, address: entry.address, mine: false });
  }
  ...
  const statuses = await mapWithConcurrency(
    ...
    STATUS_CONCURRENCY,
    (item) =>
      fetchMultisigStatus(item.address, MULTISIG_CODE, IS_TESTNET, myAddress),
  );
  if (seq !== daoRenderSeq) return;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    html += daoMultisigItemHTML(item.name, item.address, statuses[i]);
  }
...
const renderCurrentOrderInfo = (): void => {
  ...
  $("#order_tonBalance").innerText = fromNano(tonBalance) + " GRAM";
  ...
  if (actions.length === 0) {
    $("#order_actionsTitle").innerText = "Нет действий";
  } else if (actions.length === 1) {
    $("#order_actionsTitle").innerText = "Одно действие:";
  } else {
    $("#order_actionsTitle").innerText = actions.length + " действий:";
  }
...
const validateValue = (
  ...
  const parseBigInt = (inputAmount: string): ValidatedValue => {
    try {
      ...
      if (units <= 0) {
        return makeError("Введите положительную сумму");
      }
  ...
  switch (fieldType) {
    ...
    case "Address":
      if (!Address.isFriendly(value)) {
        return makeError("Некорректный адрес");
      }
    ...
    case "URL":
      if (!value.startsWith("https://")) {
        return makeError("Некорректный URL");
      }
...
const checkJettonMinterNextAdmin = async (values: {
  ...
  try {
    ...
    if (
      ...
      return {
        error:
          "Мультикошелек не является следующим администратором этого жетона",
      };
    }
...
const checkExistingOrderId = async (
  ...
  try {
    ...
    console.error(e);
    return { error: "Возможна ошибка соединения" };
  }
};
...
const orderTypes: OrderType[] = [
...

  {
    ...
    fields: {
      ...
      amount: {
        name: "Количество жетонов (в единицах)",
        type: "Jetton",
      },
      toAddress: {
        name: "Адрес получателя",
...

  {
    ...
    fields: {
      ...
      newAdminAddress: {
        name: "Адрес нового администратора",
        type: "Address",
      },
    },
    check: checkJettonMinterAdmin,
  ...
  {
    name: "Вестинг: Отправить из вестинго (0.1 GRAM за газ)",
    fields: {
      vestingAddress: {
        name: "Адрес вестинга",
        type: "Address",
...
const setNewOrderMode = (mode: "fill" | "confirm") => {
  if (mode == "fill") {
    ...
    $("#newOrder_backButton").innerHTML = "Отмена";
  }
  newOrderMode = mode;
};
...
$("#newMultisig_createButton").addEventListener("click", async () => {
  ...
  if (
    ...
    alert("Порог: некорректное число");
    return;
  }
  ...
  if (newMultisigMode === "create") {
    ...
    if (
      ...
      if (
        !confirm(
          "У вас есть ожидающие заявки, и вы изменяете конфигурацию мультикошелька. Эти ожидающие заявки больше не смогут быть исполнены. Продолжить?",
        )
      ) {
        return;
      }
